...
int main(int argc, char **argv){
    ...
    std::cin.getline(searchResult, 100);  

    std::cout << "You searched for: " << searchResult << std::endl;
    
    std::string searchQuery(searchResult);
    auto stops = network.search(searchQuery);  

    // Gefundene Stops ausgeben
    for (const auto& stop : stops) {
        std::cout << "Stop ID: " << stop.id << ", Name: " << stop.name << std::endl;
    }
    /*
    //for Agencies
